  ...
  for (let i = 1; i < allChildren.length; i += 1) {
    if (typeof productsList[i - 1] !== 'undefined') {
      ...
      if (i === 2 && buttonText2) {
        ...
        buylink.innerHTML += `<a class="red-buy-button await-loader prodload prodload-${onSelectorClass} ${!btn3link ? 'buylink-' + onSelectorClass : ''}" ${btn3link ? `href="${btn1link}"` : ''} data-type="buy-btn">${btn3txt}</a>`;
      } else if (buttonText) {
        // default
