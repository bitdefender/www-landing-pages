  ...
  if (productsAsList.length) {
    ...
    productsAsList.forEach((product, idx) => {
      ...
      block.querySelectorAll(`.c-productswithvpn2 > div:nth-child(${idx + 1}) table:first-of-type`).forEach((item) => {
        item.classList = `await-loader prod-load prod-load-${onSelectorClass}`;
      });
      // old price:
      ...
      tableBuybtn.innerHTML = `<div class="buy_box buy_box${idx + 1}"><a href='#' title='Bitdefender ${onSelectorClass}' class='red-buy-button await-loader prod-load prod-load-${onSelectorClass} buylink-${onSelectorClass}'>${tableBuybtn.innerText}</a></div>`;
      ...
      if (tableVpn.innerText.indexOf('0') !== -1) {
        ...
        vpnDiv.classList = `vpn_box await-loader prod-load prod-load-${onSelectorClass}`;
